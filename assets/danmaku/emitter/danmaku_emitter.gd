...
# 特效
@export_group("BulletEffect")
@export_color_no_alpha var bullet_color: Color = Color.WHITE ## 颜色
...
@onready var bullets_multi_mesh_instance: MultiMeshInstance3D = $BulletsMultiMeshInstance3D
...
func _ready() -> void:
	if not Engine.is_editor_hint():
		#emitter_mesh_display.queue_free()
...
# func _draw_bullet(_delta: float) -> void: # 绘制子弹的函数
...
func _delete_bullet(bullet: Bullet) -> void: # 删除指定的弹幕
	bullets.erase(bullet)
	Global.danmaku_manager.particles.add_break_group(bullet.global_position, bullet.color, bullet.bullet_radius*14)
	bullet.queue_free()
