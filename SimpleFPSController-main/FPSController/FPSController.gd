...
func _physics_process(delta):
	...
	if not _handle_noclip(delta) and not _handle_ladder_physics():
		...
		if not snap_up_stairs_check(delta):
			# Because snap_up_stairs_check moves the body manually, don't call move_and_slide
			...
			# 因为 snap_up_stairs_check 是手动移动肢体的，所以不要调用 move_and_slide
			# 这应该没问题，因为我们已经通过 body_test_motion 确保它不会
			# 除了移动到的楼梯外，不会与其他任何物体发生碰撞。
			_push_away_rigid_bodies() # Call before move_and_slide()   在 move_and_slide() 之前调用
			move_and_slide()
...
func save_camera_pos_for_smoothing():
	if saved_camera_global_pos == null:
		saved_camera_global_pos = %CameraSmooth.global_position

func _slide_camera_smooth_back_to_origin(delta):
	if saved_camera_global_pos == null: return
	%CameraSmooth.global_position.y = saved_camera_global_pos.y
	# Clamp incase teleported
	%CameraSmooth.position.y = clampf(%CameraSmooth.position.y, -CROUCH_TRANSLATE, CROUCH_TRANSLATE)
...
func snap_down_to_stairs_check() -> void:
	...
	# Since it is called after move_and_slide, last_frame_was_on_floor should still be current frame number.
	# After move_and_slide off top of stairs, on floor should then be false. Update raycast incase it's not already.
	# 根据教程稍作修改。我没有注意到任何视觉差异，但我认为这是正确的。
	# 因为是在 move_and_slide 之后调用的，所以 last_frame_was_on_floor 应该仍然是当前帧数。
	# 在从楼梯顶部 move_and_slide 之后，on floor 应该为 false。更新光线投射（raycast），以防尚未更新。
...
func snap_up_stairs_check(delta) -> bool:
	...
	# 如果试图跳跃，则不要捕捉楼梯，如果没有移动，也无需检查前方是否有楼梯
	...
	# 在我们预计移动到的位置稍上方，朝地板方向运行一个 body_test_motion 运动。
	# 我们在上方留出一些间隙，以确保玩家有足够的空间。
	# 如果它碰到的台阶 <= MAX_STEP_HEIGHT，我们就可以将玩家传送到台阶的顶部
	# 随着他们的预定动作向前移动。
	...
	and (down_check_result.get_collider().is_class("StaticBody3D") or down_check_result.get_collider().is_class("CSGShape3D"))):
		...
		# 注意，我把 step_height <= 0.01 放进去只是因为我注意到它能防止一些物理故障
		# 0.02 是通过反复试验得出的。太多，有时会卡在楼梯上。太小，碰到天花板时会抖动。
		# 普通角色控制器（包括颠簸控制器和默认控制器）似乎可以处理 0.1 以上的台阶。
