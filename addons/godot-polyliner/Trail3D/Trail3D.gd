...
enum SamplingMode { # 采样模式
	Process,
	PhysicsProcess,
	None
}

enum TangentAxis { # 切线轴
	X,
	Y,
	Z
}
...
@export_range(0, 1000) var max_points: int = 10: set = set_max_points # (int,0,1000)
@export_range(0.0, 1.0, 0.005) var damping: float = 0.5 # 生成的阻尼
@export var tangent_axis: TangentAxis = TangentAxis.X # 切线轴
@export var skip_frames: int = 0 # (int,0,20)
...
var points = []
var damped_transform = Transform3D() # 阻尼变换
var _mesh_xform: Transform3D = Transform3D.IDENTITY # 用于存储MeshInstance3D的变换


func _ready():
	damped_transform = global_transform # 初始化时设置变换
	_mesh_instance.set_as_top_level(true) # 将网格实例设置为顶层对象
	set_sampling_mode(sampling_mode) # 设置采样模式
	set_max_points(max_points) # 设置最大点数
	_set_material(material) # 设置材质
	_linegen.render_mode = Mesh.PRIMITIVE_TRIANGLES # 设置线条生成器的渲染模式为三角形
